...
freq = 100  # Герц, частота посылки управляющих команд аппарату
...
def subscribe_on_topics():
    ...
    # /path_generator/_central
    # /path_generator/_walls
...
def my_example(pt, n, dt):
    mc_takeoff(pt, n, dt)
    if dt > 1:
        # print(data[n].get("local_position/pose").pose.position)
        # print(data.get("/path_generator/_walls"))
        set_pos(pt, 0, 0, n)
